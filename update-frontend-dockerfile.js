...
async function updateDockerCompose() {
  try {
    ...
    // Create a custom Dockerfile for the frontend that skips the build step
...
# Create a custom next.config.js that skips the build step for pages that require Supabase
RUN echo "const nextConfig = {" > next.config.js.new \\
    && echo "  env: {" >> next.config.js.new \\
    && echo "    NEXT_PUBLIC_SUPABASE_URL: 'http://supabasekong-akowwgwwwwcwossg4cgwk0ok.m2w.io:8000'," >> next.config.js.new \\
    && echo "    NEXT_PUBLIC_SUPABASE_ANON_KEY: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZS1kZW1vIiwicm9sZSI6ImFub24iLCJleHAiOjE5ODM4MTI5OTZ9.CRXP1A7WOeoJeXxjNni43kdQwgnWNReilDMblYTn_I0'," >> next.config.js.new \\
    && echo "  }," >> next.config.js.new \\
    && echo "  output: 'standalone'," >> next.config.js.new \\
    && echo "  experimental: {" >> next.config.js.new \\
    && echo "    serverComponentsExternalPackages: ['@prisma/client']," >> next.config.js.new \\
    && echo "  }," >> next.config.js.new \\
    && echo "};" >> next.config.js.new \\
